...
export const formatAddress = (address, l = 6) =>
	`${address.slice(0, l + 2)}...${address.slice(
		address.length - l,
		address.length
	)}`
	
export const formatNumber = (number, pow = 0) => pow === 0 ? number : Math.floor(number * 10 ** pow) / 10 ** pow
